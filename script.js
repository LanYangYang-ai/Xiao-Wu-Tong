...
function startQuiz(sid){var Q=QUIZ[sid];if(!Q||Q.length<1)return;
...
var ids=[];for(var i=0;i<Math.min(3,av.length);i++){var r2=Math.floor(Math.random()*av.length);ids.push(av[r2]);av.splice(r2,1);}qc(sid,ids);}
...
function sa(s){var c=QC;var q=QUIZ[c.sid][c.ids[c.idx]];var ok=s===q.a;if(ok)c.correct++;
...
function sr(){var c=QC;var sigs=["你今天的物理力场，强得像个黑洞。","误差是测量的灵魂，下次会更精确。","能量守恒，错题守恒，但你不会永远错。","物理学家也曾对着数据发呆，你并不孤单。"];
...
var _orlt=renderLogicTree;renderLogicTree=function(){_orlt();setTimeout(function(){document.querySelectorAll(".step-card").forEach(function(c){if(c.querySelector(".quiz-btn"))return;var sid=c.getAttribute("data-step");if(!sid||!QUIZ[sid])return;var b=document.createElement("button");b.className="quiz-btn";b.textContent="📝 来测三道这个步骤的核心卡点";b.onclick=function(){startQuiz(sid);};c.appendChild(b);});},200);};
var BW=["搞笑","鬼畜","游戏","娱乐","网红","吃鸡","王者","抖音","快手","电影","动漫","我的世界","Minecraft","模组","原神","英雄联盟","第五人格"];
function bs(kw,cb){
fetch("https://api.allorigins.win/raw?url="+encodeURIComponent("https://api.bilibili.com/x/web-interface/wbi/search/type?search_type=video&keyword="+encodeURIComponent(kw+" 高中物理")+"&page=1")).then(function(r){return r.json();}).then(function(d){
