...
var h=document.getElementById('videoHint');var g=document.getElementById('videoGrid');if(h)h.textContent='????: '+nodeName;if(g)g.innerHTML='';
var bw=['??','??','??','??','??','??','??','??','??','??','??','????','Minecraft','??','??','????','????'];
var u='https://api.bilibili.com/x/web-interface/wbi/search/type?search_type=video&keyword='+encodeURIComponent(biliKeywords+' ????')+'&page=1';
function hd(d){if(d.code===0&&d.data&&d.data.result&&d.data.result.length>0){var vs=d.data.result.filter(function(it){var t=(it.title||'').replace(/<[^>]+>/g,'');for(var i=0;i<bw.length;i++){if(t.indexOf(bw[i])>=0)return false;}return true;}).sort(function(a,b){return(b.play||0)-(a.play||0)}).slice(0,20).map(function(it){return{id:it.bvid||'',title:(it.title||'').replace(/<[^>]+>/g,''),cover:it.pic||'',author:it.author||'??',url:'https://www.bilibili.com/video/'+(it.bvid||''),view:it.play||0,duration:it.duration||'00:00'};});if(window.renderVideos)renderVideos(vs);if(h)h.textContent='?? '+vs.length+' ???';}}
fetch(u).then(function(r){if(r.ok)return r.json();throw Error();}).then(function(d){hd(d);}).catch(function(){fetch('https://api.allorigins.win/raw?url='+encodeURIComponent(u)).then(function(r){return r.json();}).then(function(d){hd(d);}).catch(function(){fetch('https://corsproxy.io/?url='+encodeURIComponent(u)).then(function(r){return r.json();}).then(function(d){hd(d);}).catch(function(){if(h)h.textContent='B?API???';var a=document.createElement('a');a.href='https://search.bilibili.com/all?keyword='+encodeURIComponent(biliKeywords+' ????');a.target='_blank';a.textContent='?B????????';a.style.cssText='display:block;text-align:center;padding:12px;color:#3182ce;font-size:15px;margin-top:16px;text-decoration:none';if(g){g.innerHTML='';g.appendChild(a);}});});});
}
// ==================== 高中物理 · 逻辑脉络图 ====================
// 核心原则：理性 · 从容 · 长期主义
// 所有逻辑树数据硬编码在前端，运行时零API消耗
